from dataclasses import dataclass, field, InitVar, astuple, asdict
from typing import Dict, List, Any, Optional, Union, Tuple, Iterator, Iterable
...
class ActionFeature(BaseFeature):
    ...
    context_segments: List[int] = field(default_factory=list)  #= context["segment_ids"]
...
class CompletionFeature(BaseFeature):
    ...
    context_token: List[List[int]] = field(init=False)
    context_segment: Any = field(init=False)
    context_mask: Any = field(init=False)

    label_ids: InitVar[Dict]

    intent_id: int = field(init=False)
    nextstep_id: int = field(init=False)
    action_id: int = field(init=False)
    value_id: int = field(init=False)
    utt_id: int = field(init=False)
